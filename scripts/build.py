...
def csv_bytes(records: list[dict]) -> bytes:
    ...
    for record in records:
        ...
        row["official_url"] = record.get("congress_url")
...
def search_index(records: list[dict]) -> list[dict]:
    fields = ("record_id", "measure_id", "title", "entity_type", "scope", "primary_category", "status", "lifecycle_status", "summary")
    result = []
    for record in records:
...
def record_page(record: dict) -> bytes:
    ...
    content = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(record['measure_id'])} — {esc(record['title'])}</title><style>body{{font:16px/1.55 system-ui;max-width:860px;margin:40px auto;padding:0 20px;color:#17252b}}a{{color:#075f72}}dt{{font-weight:700;margin-top:12px}}dd{{margin-left:0}}small{{color:#52656d}}</style></head><body><p><a href="../index.html">← Tracker</a></p><main><p>{esc(record['entity_type'].title())} · {esc(record['jurisdiction']['level'].title())} · {esc(record['lifecycle_status'])}</p><h1>{esc(record['measure_id'])}: {esc(record['title'])}</h1><p>{esc(record['summary'])}</p><dl><dt>Status</dt><dd>{esc(record['status'])}</dd><dt>Last action</dt><dd>{esc(record.get('last_action'))}</dd><dt>Primary category</dt><dd>{esc(record['primary_category'])}</dd><dt>Last verified</dt><dd>{esc(record['last_verified_date'])}</dd></dl><h2>Sources</h2><ul>{source_items}</ul><h2>Claim provenance</h2><ul>{claim_items}</ul></main></body></html>'''
...
def outputs(records_with_meta: list[dict]) -> dict[Path, bytes]:
    ...
    site = re.sub(r'(<div class="donut-label"><strong>).*?(</strong>)', rf'\g<1>{len(records)}\2', site)
